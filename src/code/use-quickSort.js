...
const useQuickSort = () => {
  ...
  const quickSortImplementation = ({ arr, parent }) => {
    ...
    const leaf = { parent, id: uid(), name: "main", pivot, element: arr, right, left };
    tree.push(leaf);
    let lastId;
    arr.forEach((element, index) => {
      ...
      lastId = uid();
      const leafMove = { parent, id: lastId, name: "sort", pivot: "", right, left };
      tree.push(leafMove);
    });
    return [
    ...
    if (tree.length === 0) {
      ...
      return result.export((data) => {
        return {
          name: `${data.values.id}`,
          attributes: {
            tag: data.values.name === "main" ? `${data.values.name} -(${data.values.pivot}) [${data.values.element.join(",")}]` : `${data.values.name} -(${data.values.pivot}) [${data.values.right.join(",")}] [${data.values.left.join(",")}]`,
          },
        };
      });
    }
